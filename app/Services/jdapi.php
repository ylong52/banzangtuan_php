...
// die;
    ...
    try {
        $bizParams = [
            'goodsReqDTO' => [
        //         'keyword' => '【京东】https://3.cn/2iF-z6i8 「创维电视85A4E 安装版【固定挂架送装一体】护眼电视机120Hz高刷媲美mini led 超薄巨幕大屏游戏电视    」
        // 点击链接直接打开',
                'keyword'=> $keyword,
                'pageIndex' => 1,
                'pageSize' => 2,
        ...
        if ($result['success']) {
            ...
            if ($data['jd_union_open_goods_query_responce']['code']==0 && $data['jd_union_open_goods_query_responce']['code']==0 ) {
                ...
                if ($goodsInfo['couponCommission']) {
                    $goodsInfo['discount'] = round($goodsInfo['couponCommission']/10,2);
                }
                $goodsInfo['discount'] = 1.2; //测试数据
                $goodsInfo['amount'] = calculateAdjustedY($goodsInfo['discount']);  //加工佣金
//                 echo "<pre> 149 >>>";
// var_dump($goodsInfo['amount'] ); die;      
...
function calculateAdjustedY($x) {
    ...
    if ($x < 1) {
        $goodsInfo['discount'] = 1;
        return ceil(20 / $goodsInfo['discount']);
